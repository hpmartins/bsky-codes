...
import AtpAgent, { AppBskyFeedPost, BskyAgent } from '@atproto/api';
...
import {
    storeBlocksHistogram,
    storeFollowsHistogram,
    storeLikesHistogram,
    storePostsByLang,
    storePostsHistogram,
    storeProfilesHistogram,
    storeRepostsHistogram,
    storeTopBlocked,
    storeTopPosters
} from './tasks/stats';
...
async function run() {
    ...
    const cfg: AppConfig = {
        ...
        bskyDid2: maybeStr(process.env.BOLBI_BSKY_DID) ?? '',
        bskyPwd2: maybeStr(process.env.BOLBI_BSKY_PASSWORD) ?? '',
    ...
    const agent2 = new BskyAgent({ service: 'https://bsky.social/' });
    await agent2.login({ identifier: cfg.bskyDid2, password: cfg.bskyPwd2 });
