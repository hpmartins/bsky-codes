...
def run(name, sio, stream_stop_event=None):
    while stream_stop_event is None or not stream_stop_event.is_set():
        try:
            _run(name, sio, stream_stop_event)
        except FirehoseError as e:
            # here we can handle different errors to reconnect to firehose
            raise e


def _run(name, sio, stream_stop_event=None):
    ...
    def on_message_handler(message: firehose_models.MessageFrame) -> None:
        ...
        sio.emit("firehose", {"data": _get_ops_by_type(commit)})
