...
<div class="text-center">
    ...
    <p class="text-lg">
        {$t('features.languages.tableTitle')} - {$t('features.languages.tableSubtitle')}
    </p>
</div>
            ...
            <table class="table table-xs table-zebra">
                <thead>
                    <tr class="text-center text-lg text-primary">
                        <th>#</th>
                        <th title={$t('features.common.language')}
                            >{$t('features.common.language')}</th
                        >
                ...
                <tbody>
                    {#each totalTablePagData as row (row.lang)}
                        <tr class="text-center">
                            <td>{row.idx}</td>
                            <td
                                >{row.lang ?? 'none'}{row.langName.length > 0
                                    ? ` (${row.langName})`
                                    : ''}</td
                            >
                            <td>{row.count}</td>
